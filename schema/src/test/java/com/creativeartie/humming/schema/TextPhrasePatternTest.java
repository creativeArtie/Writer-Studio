...
class TextPhrasePatternTest extends PatternTestBase<TextPhrasePatterns.LineTextPart> {
    @BeforeAll
    static void setUpBeforeClass() throws Exception {
        splitPrintPattern(TextPhrasePatterns.BASIC.matcher("abc"));
    }

    @Test
    void testFormated() {
        final Matcher match = TextPhrasePatterns.BASIC.matcher("*`abc`*avd");
        match.find();
    ...
    @Test
    void testRef() {
        final Matcher match = TextPhrasePatterns.BASIC.matcher("{!avd}{^add}{dadd}add");
        match.find();
