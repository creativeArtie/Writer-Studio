...
public class IDOrderDebug {
    ...
    public void longList(){
        ...
        SortedMap<CatalogueIdentity, CatalogueData> test = 
            doc.getCatalogue().getCategory("b");
        assertEquals("Wrong size.", 8, test.size());
        
        int i = 1;
        for(Map.Entry<CatalogueIdentity, CatalogueData> child: 
            test.entrySet())
        {
            CatalogueIdentity id = null;
            id = new CatalogueIdentity(i == 8? Arrays.asList("b", "a"):
                Arrays.asList("b"), String.valueOf(i));
            assertEquals("Wrong id.", child.getKey(), id);
            i++;
        }
    }
}
